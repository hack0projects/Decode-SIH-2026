...
async function runPipeline(jobId, rawText) {
  ...
  try {
    ...
    console.log('\n[1/5] Gemini -- generating unlimited-scene script...');
    emitProgress(jobId, 5, 'gemini', 'AI reading your content and planning scenes...');
    const model  = genAI.getGenerativeModel({ model: 'gemini-3.6-flash' });
    const prompt = `You are a world-class educational video scriptwriter for CodeSeekho, an AI edtech platform.

Your task: Read the TEXT below carefully and convert EVERY concept, topic, and subtopic into a comprehensive multi-scene video script. The AI avatar teacher explains each idea step by step.

OUTPUT RULES:
1. Output ONLY valid raw JSON. No markdown, no backticks, no explanation, no trailing commas.
2. ASCII characters only. No smart quotes, no en-dash. Use standard " and -.
3. Escape all newlines inside strings as \\n

SCHEMA:
{
  "title": "Engaging descriptive lesson title (max 65 chars)",
  "duration": 180,
  "language": "Python",
  "scenes": [ ... ]
}
...
SCENE TYPES:
A) intro: { "type": "intro", "text": "Short Headline Here\\nDetailed explanation in 3-4 sentences. Cover the concept thoroughly. Use analogies and real-world examples. Each sentence should add new understanding." }
B) code: { "type": "code", "code": "# Well-commented code\\n# Each concept demonstrated\\nprint('example')" }
C) visual: { "type": "visual", "animation": "forLoopIterator" } OR { "type": "visual", "animation": "whileCounter" }
   RULE: animation MUST be exactly "forLoopIterator" or "whileCounter" -- no other values.

CONTENT RULES:
1. NO TIME LIMIT. Generate as many scenes as needed to cover ALL content. Minimum 10 scenes.
2. Every major concept, subtopic, and example from the input MUST appear in at least one scene.
3. Each intro text: 1 short headline + 3-4 educational sentences in the body.
4. Include at least 3 code scenes with real working commented code.
5. Include at least 2 visual scenes.
6. Order: intro(overview) -> code -> visual -> intro -> code -> intro -> ... -> intro(summary + takeaways)
...
JSON only:`;
    ...
    while (retries > 0) {
      try {
        ...
        retries--;
        console.warn(`   Gemini API Error: ${err.message}. Retrying in ${delay/1000}s... (${retries} left)`);
        emitProgress(jobId, 10, 'gemini', `API busy, retrying in ${delay/1000}s...`);
        if (retries === 0) throw Object.assign(new Error(`Gemini API failed: ${err.message}`), { phase: 'gemini' });
        await new Promise(r => setTimeout(r, delay));
        delay *= 2;
      }
    }
    const rawResp = geminiResult.response.text().trim();
    let script;
    try {
      script = extractJSON(rawResp);
    } catch(e) {
      console.error('Raw Gemini:\n', rawResp.slice(0, 600));
      throw Object.assign(new Error(`Malformed JSON: ${e.message}`), { phase: 'gemini' });
    }
